...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'JarvisionProject'
...
def transcribe_chunks(audio_chunks):
    ...
    # Create a temporary file to store the audio_chunks_data
    with tempfile.NamedTemporaryFile(suffix=".wav") as temp_audio_file:
        audio_chunks.export(temp_audio_file.name, format="wav")
...
def index():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            transcript = pipeline(audio_path, task="transcribe",  return_timestamps=True)
            transcription_time = transcript["chunks"]
        ...
        return render_template('result.html', transcript=transcript, transcription_time=transcription_time)
